<?php echo tag( isset($total) && $total > 1 ? "li" : "div", array("class"=>"hentry post ", "id"=>"story_{$story['id']}"), true);?>
  ...
  <?php include_partial('story/storyActions', array('story' => $story));?>
   ...
   <h3 class="entry-title">
     ...
     <?php else:?>
       <?php echo link_to_story(preg_replace('/\s+?(\S+)?$/', '', substr($story->title, 0, 50)), $story, array("title" => $story->title));?>
     <?php endif; ?>
       <?php
  ...
  <?php if($story->type == Story::TYPE_NORMAL):?>
    <div class="screenshot">
        ...
        if($source){
            ...
            echo img_tag("http://stromboli.yigg.de/?url=" . $story->external_url, array("width" =>  338, "height" => 88));
        }
